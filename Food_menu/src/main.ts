class Menu {
    ...
    public newMenu: Array<any> = [];
    constructor() {}
    public getMenu(cata:string) {
      const here = this
    ...
    public showMenu(cata:string = 'all'):void {
      this.getMenu(cata).then(()=>{
        ...
        let getMenus = this.newMenu.map((e:any)=>{
        ...
        const menuHtml:string = getMenus.join('\n')
  ...
  // select items
  const section = document.querySelector('.section-center') as HTMLDivElement
  ...
  filterBtns.forEach((btn)=>{
    btn.addEventListener('click',(e)=>{
      ...
      menu.showMenu(category as string);
    })
  })
